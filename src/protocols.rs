...
use bevy::prelude::{Assets, Color, Entity, Handle, Query, Res, ResMut, Time, Transform};
...
pub fn update_networks(
    ...
    mut sat_query: Query<(Entity, &Transform, &mut Satellite, &mut Handle<StandardMaterial>)>,
    ...
    for sat in sat_list.iter_mut() {
        if has_line_of_sight(sat.pos, station.position, earth_radius) {
            sat.has_path = true;
    ...
    while let Some(current_entity) = queue.pop() {
        ...
        for i in 0..sat_list.len() {
            if !sat_list[i].has_path {
                ...
                if dist <= max_isl_distance && has_line_of_sight(current_pos, current_pos, earth_radius) {
